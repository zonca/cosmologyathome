...
! perturbation equations, and InitialPower which provides the initial power spectrum.
      ...
      interface
        function boinc_time_to_checkpoint
          ...
          integer boinc_time_to_checkpoint
        end function boinc_time_to_checkpoint
      end interface
...
contains  
      ...
      real(8) :: lower
      real(8) :: upper
      real(8) :: diff, q_ix_max
      integer is_standalone
...
!      ***note that !$ is the prefix for conditional multi-processor compilation***
      ...
      if (CP%WantCls) then
         ...
         if (CP%WantScalars) then
             ThisCT => CTransScal
             !! >> Added for CAMB checkpointing
             call check_point_init('scalar')
         else if (CP%WantVectors) then
             ThisCT => CTransVec
             !! >> Added for CAMB checkpointing
             call check_point_init('vector')
         else
             ThisCT => CTransTens
             !! >> Added for CAMB checkpointing
             call check_point_init('tensor')
         end if
...
!     integrating the sources over time and over k.
      ...
      !! >> Added for CAMB checkpointing
      call check_point_resume(num_ks_resume,perc_resume,ThisCT%Delta_p_l_k)
...
!!        do q_ix=1,ThisCT%q%npoints
         do q_ix = num_ks_resume + 1, ThisCT%q%npoints
            call SourceToTransfers(q_ix)

            if (is_standalone /= 0) then
              write(*,*) "Fraction done: ", (((REAL(q_ix) / q_ix_max) * diff) + lower)
            else
              call boinc_fraction_done(((REAL(q_ix) / q_ix_max) * diff) + lower)
            end if

            if (mod(q_ix, ThisCT%q%npoints/NUM_CHECK_POINTS) == 0) then
              if (boinc_time_to_checkpoint() /= 0) then
                call check_point_record(q_ix, ThisCT%Delta_p_l_k)
                call boinc_checkpoint_completed()
              end if
            end if

            if (mod(q_ix,ThisCT%q%npoints/10) == 0) then
               perc_resume = perc_resume + 10
            end if

            !! >> Added for CAMB checkpointing
            !if (mod(q_ix,ThisCT%q%npoints/NUM_CHECK_POINTS) == 0) &
            !             call check_point_record(q_ix,ThisCT%Delta_p_l_k)
